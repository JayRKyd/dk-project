...
export default function LadyReviews() {
  ...
  const loadReviews = async () => {
    try {
      setLoading(true);
      if (user?.id) {
        const reviews = await ladyService.getReceivedReviews(user.id);
        setReviews(reviews);
      }
    } catch (error) {
  ...
  const loadStats = async () => {
    try {
      if (user?.id) {
        const stats = await ladyService.getReviewStats(user.id);
        setStats(stats);
      }
    } catch (error) {
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-pink-500 text-white text-sm rounded-lg hover:bg-pink-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
                          {submittingReply ? (
                            <>
      ...
      {reviews.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600">
            When clients write reviews about their experiences with you, they'll appear here.
          </p>
        </div>
      )}
    </div>
